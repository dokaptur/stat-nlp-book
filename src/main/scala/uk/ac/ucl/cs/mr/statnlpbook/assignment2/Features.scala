...
object Features {
  ...
  def myTriggerFeatures(x: Candidate, y: Label): FeatureVector = {
    ...
    if (thisSentence.tokens.length > begin + 1) {
      val after = thisSentence.tokens(begin+1)
      feats += FeatureKey("bigram after", List(after.word, token.word, y)) -> 1.0 //word feature
    }
...
//    mentions.foreach(m => {
//      feats += FeatureKey("mention in sentence", List(m.label, y)) -> 1.0
//    })
